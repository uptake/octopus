...
vcs_client = case options[:scm_provider].downcase
             when 'bitbucket'
               Octopus::Clients::Bitbucket.new(options[:scm_url], options[:username], options[:password])
             else
...
command = case options[:command]
          ...
          when Octopus::Options::COMMAND_DEFAULT_BRANCHES
            Octopus::Commands::ReportDefaultBranches.new(vcs_client, options[:thread_count])
          end
